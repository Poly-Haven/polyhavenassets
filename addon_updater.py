...
class SingletonUpdater:
    ...
    def create_backup(self):
        ...
        if self._backup_ignore_patterns is not None:
            try:
                shutil.copytree(
                    self._addon_root, tempdest, ignore=shutil.ignore_patterns(*self._backup_ignore_patterns)
                )
            except:
            ...
            for file in files:
                ...
                if os.path.isfile(dest_file):
                    ...
                    if replaced:
                        ...
                        self.print_verbose(
                            "Pattern not matched to {}, not overwritten".format(os.path.basename(dest_file))
                        )
                else:
    ...
    def get_json_path(self):
        ...
        """
        json_path = os.path.join(self._updater_path, "{}_updater_status.json".format(self._addon_package))
    ...
    def save_updater_json(self):
        ...
        if not os.path.isdir(os.path.dirname(jpath)):
            print("State error: Directory does not exist, cannot save json: ", os.path.basename(jpath))
            return
        try:
...
class BitbucketEngine:
    ...
    def parse_tags(self, response, updater):
        if response is None:
            return list()
        return [
            {"name": tag["name"], "zipball_url": self.get_zip_url(tag["name"], updater)} for tag in response["values"]
        ]
...
class GitlabEngine:
    ...
    def parse_tags(self, response, updater):
        if response is None:
            return list()
        return [
            {"name": tag["name"], "zipball_url": self.get_zip_url(tag["commit"]["id"], updater)} for tag in response
        ]
